...
/*    // Cacheability attribute check
	for(sec_idx = 0; sec_idx < 256; sec_idx++)
	{
		...
		if ((PA_TO_PH_BLOCK(0x87800000) < sec_block) && (sec_block < PA_TO_PH_BLOCK(0x87900000))){
...
int dmmu_create_L1Pt(addr_t l1_base_pa_add)
{
...
/*    // Cacheability attribute check
	for(sec_idx = 0; sec_idx < 256; sec_idx++)
	{
		uint32_t sec_block = PA_TO_PH_BLOCK(START_PA_OF_SECTION(l1_sec_desc)) | sec_idx;
		if ((PA_TO_PH_BLOCK(0x87800000) < sec_block) && (sec_block < PA_TO_PH_BLOCK(0x87900000))){
			//printf("Section block %x \n", sec_block);
			if (l1_sec_desc->c != 1)
				return ERR_MMU_NOT_CACHEABLE;
 ...
 *  ------------------------------------------------------------------- */
uint32_t dmmu_unmap_L1Pt_entry (addr_t  va)
{
    uint32_t l1_base_add;
    uint32_t l1_idx;
	uint32_t l1_desc_pa_add;
...
/*	if ((PA_TO_PH_BLOCK(0x87800000) < pg_desc->addr) && (pg_desc->addr < PA_TO_PH_BLOCK(0x87900000))){
...
uint32_t dmmu_create_L2Pt(addr_t l2_base_pa_add)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
 ...
 *  -------------------------------------------------------------------*/
int dmmu_map_L1Pt_pt(addr_t va, addr_t l2_base_pa_add, uint32_t attrs)
{
    uint32_t l1_base_add;
    uint32_t l1_idx;
    uint32_t l1_desc_pa_add;
    ...
    //printf("Hyper L2 base in L1 map %x \n", l2_base_pa_add);
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
    CacheDataCleanInvalidateAll();
 ...
 *  -------------------------------------------------------------------*/
int dmmu_map_L2Pt_entry(addr_t l2_base_pa_add, uint32_t l2_idx, addr_t page_pa_add, uint32_t attrs)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
 ...
 *  -------------------------------------------------------------------*/
int dmmu_unmap_L2Pt_entry(addr_t l2_base_pa_add, uint32_t l2_idx)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
 ...
 *  -------------------------------------------------------------------*/
int dmmu_free_L2Pt(addr_t l2_base_pa_add)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
 ...
 *  -------------------------------------------------------------------*/
//#define SW_DEBUG
int dmmu_switch_L1Pt(addr_t l1_base_pa_add)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
 ...
 *  ------------------------------------------------------------------- */
int dmmu_free_L1Pt(addr_t l1_base_pa_add)
{
#ifdef AGGRESSIVE_FLUSHING
    isb();
    mem_mmu_tlb_invalidate_all(TRUE, TRUE);
...
int dmmu_handler(uint32_t p03, uint32_t p1, uint32_t p2)
{
    ...
    case CMD_CREATE_L1_PT:
    	return dmmu_create_L1Pt(p1);
    case CMD_MAP_L1_SECTION:
    	return dmmu_map_L1_section(p1,p2,p3);
    case CMD_UNMAP_L1_PT_ENTRY:
    	return dmmu_unmap_L1Pt_entry(p1);
    case CMD_CREATE_L2_PT:
    	return dmmu_create_L2Pt(p1);
    case CMD_MAP_L1_PT:
    	return dmmu_map_L1Pt_pt(p1, p2, p3);
    case CMD_MAP_L2_ENTRY:
    	p3 = p03 & 0xFFFFFFF0;
    	uint32_t idx = p2 >> 20;
    	uint32_t attrs = p2 & 0xFFF;
    	return dmmu_map_L2Pt_entry(p1, idx, p3, attrs);
    ...
    case CMD_UNMAP_L2_ENTRY:
    	return dmmu_unmap_L2Pt_entry(p1, p2);
    case CMD_FREE_L2:
    	return dmmu_free_L2Pt(p1);
    case CMD_FREE_L1:
    	return dmmu_free_L1Pt(p1);
    case CMD_SWITCH_ACTIVE_L1:
    	return dmmu_switch_L1Pt(p1);
    default:
