...
void hypercall_dyn_switch_mm(addr_t table_base, uint32_t context_id)
{
	...
	if(dmmu_switch_L1Pt(table_base & L1_BASE_MASK)){
		printf("\n\tCould not switch MM\n");
		dmmu_switch_L1Pt(table_base & L1_BASE_MASK);
	}
...
void hypercall_dyn_free_pgd(addr_t *pgd_va)
{
    ...
    for(i = l2_entry_idx; i < l2_entry_idx + 4; i++){
    	if(dmmu_unmap_L2Pt_entry(table2_pa & L2_BASE_MASK, i))
    		printf("\n\tCould not unmap L2 entry in new PGD\n");
    }

    if(dmmu_free_L1Pt(LINUX_PA((addr_t)pgd_va)))
    	printf("\n\tCould not unmap L1 PT in free pgd\n");

    for(i = l2_entry_idx; i < l2_entry_idx + 4; i++, page_pa+=0x1000){
    	if(dmmu_map_L2Pt_entry(table2_pa & L2_BASE_MASK, i, page_pa,  attrs))
...
void hypercall_dyn_new_pgd(addr_t *pgd_va)
{
    ...
    COP_WRITE(COP_SYSTEM, COP_TLB_INVALIDATE_MVA, linux_va);
		...
		if(dmmu_unmap_L1Pt_entry((addr_t)linux_va))
		...
		if(dmmu_map_L1Pt_pt((addr_t)linux_va, table2_pa, attrs))
        ...
        for(i = table2_idx; i < end;i++, page_pa+=0x1000){
        	if(i >= l2_entry_idx && i < l2_entry_idx + 4){ /*4 small pages RO 16KB PGD*/
        		...
        		if(dmmu_map_L2Pt_entry(table2_pa, i, page_pa,  ro_attrs))
        			printf("\n\tCould not map L2 entry in new pgd\n");
        	}
        	else
        		if(dmmu_map_L2Pt_entry(table2_pa, i, page_pa,  attrs))
        			printf("\n\tCould not map L2 entry in new pgd\n");
        }
	...
	/*Here we already got a L2PT*/
        ...
        for(i = l2_entry_idx; i < l2_entry_idx +4;i++, page_pa+=0x1000){
        	if(dmmu_unmap_L2Pt_entry(table2_pa & L2_BASE_MASK, i))
        		printf("\n\tCould not unmap L2 entry in new PGD\n");
        	uint32_t ro_attrs = 0xE | (MMU_AP_USER_RO <<  MMU_L2_SMALL_AP_SHIFT);
        	if(dmmu_map_L2Pt_entry(table2_pa & L2_BASE_MASK, i, page_pa,  ro_attrs))
        		printf("\n\tCould not map L2 entry in new pgd\n");
	...
	if(dmmu_create_L1Pt(LINUX_PA((addr_t)pgd_va))){
...
void hypercall_dyn_set_pmd(addr_t *pmd, uint32_t desc)
{
	...
	if(l1_desc_entry & MMU_L1_TYPE_SECTION) { /*SECTION page, replace with lvl2 pages */
		...
		/*Clear the SECTION entry mapping and replace it with a L2PT */
		if(dmmu_unmap_L1Pt_entry((addr_t)desc_va))
        ...
        attrs |= (HC_DOM_KERNEL << MMU_L1_DOMAIN_SHIFT);
		if(dmmu_map_L1Pt_pt((addr_t)desc_va, table2_pa, attrs))
        ...
        for(i = table2_idx; i < end;i++, page_pa+=0x1000){
        	if(i == l2_idx +( table2_idx)){
        		...
        		if(dmmu_map_L2Pt_entry(table2_pa, i, page_pa,  l2_rw_attrs))
        			printf("\n\tCould not map L2 entry in set PMD\n");
        }
	...
	if(((l2entry_desc >> 4) & 0xff) == 3 ){
		if(dmmu_unmap_L2Pt_entry((uint32_t)l2pt_pa & L2_BASE_MASK, table2_idx+l2_idx))
			printf("\n\tCould not unmap L2 entry in set PMD\n");
		uint32_t desc_pa = MMU_L2_SMALL_ADDR(desc);
		uint32_t ro_attrs = 0xE | (MMU_AP_USER_RO <<  MMU_L2_SMALL_AP_SHIFT);

		if(dmmu_map_L2Pt_entry((uint32_t)l2pt_pa & L2_BASE_MASK, table2_idx+l2_idx, desc_pa,  ro_attrs))
			printf("\n\tCould not map L2 entry in set PMD\n");
	}
	if( desc != 0 ){
		if(dmmu_create_L2Pt(MMU_L2_SMALL_ADDR(desc))){
	...
	if(desc == 0){
		...
		isb();
#endif
		if(dmmu_unmap_L1Pt_entry(virt_transl_for_pmd))
			printf("\n\tCould not unmap L1 entry in set PMD\n");
		if(dmmu_unmap_L1Pt_entry(virt_transl_for_pmd+SECTION_SIZE))
			printf("\n\tCould not unmap L1 entry in set PMD\n");
		...
		if(dmmu_unmap_L2Pt_entry((uint32_t)l2pt_pa & L2_BASE_MASK, table2_idx+l2_idx))
					printf("\n\tCould not unmap L2 entry in set PMD\n");
		if(dmmu_free_L2Pt(MMU_L2_SMALL_ADDR((uint32_t)*pmd)))
			printf("\n\tCould not unmap L2 pt in set PMD\n");
		if(dmmu_map_L2Pt_entry((uint32_t)l2pt_pa & L2_BASE_MASK, table2_idx+l2_idx, MMU_L2_SMALL_ADDR((uint32_t)*pmd),  l2_rw_attrs))
			printf("\n\tCould not map L2 entry in set PMD\n");
		...
		if(dmmu_map_L1Pt_pt(virt_transl_for_pmd, MMU_L2_SMALL_ADDR(desc), attrs))
			printf("\n\tCould not map L1 PT in set PMD\n");
		if(dmmu_map_L1Pt_pt(virt_transl_for_pmd + SECTION_SIZE, MMU_L2_SMALL_ADDR(desc) + 0x400, attrs))
			printf("\n\tCould not map L1 PT in set PMD\n");
...
void hypercall_dyn_set_pte(addr_t *l2pt_linux_entry_va, uint32_t linux_pte, uint32_t phys_pte)
{
    ...
    uint32_t err;
    if(phys_pte != 0) {
    	if((err = (dmmu_map_L2Pt_entry(l2pt_hw_entry_pa & L2_BASE_MASK, entry_idx, MMU_L1_PT_ADDR(phys_pte),attrs)))){
    		if(err == ERR_MMU_PT_NOT_UNMAPPED){
    			/*So DMMU API does not allow changing attributes or remapping an entry if its not empty
    			 *this is a workaround */
    			dmmu_unmap_L2Pt_entry(l2pt_hw_entry_pa & L2_BASE_MASK, entry_idx);
    			dmmu_map_L2Pt_entry(l2pt_hw_entry_pa & L2_BASE_MASK, entry_idx, MMU_L1_PT_ADDR(phys_pte),attrs);
    		}
    		else {
    	...
    	/*Unmap*/
    	if(dmmu_unmap_L2Pt_entry(l2pt_hw_entry_pa & L2_BASE_MASK, entry_idx))
