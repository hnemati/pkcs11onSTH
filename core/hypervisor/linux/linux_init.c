...
void linux_init_dmmu()
{
   ...
   // dmmu_map_L1_section(guest_vstart + guest_psize, guest_pstart + guest_psize, attrs);
    ...
    for (;offset + SECTION_SIZE <= guest_psize; offset += SECTION_SIZE) {
        ...
        if((error = dmmu_map_L1Pt_pt((addr_t)guest_vstart + offset, table2_pa, page_attrs)))
        ...
        for(i = table2_idx, page_pa = offset; i < end;i++, page_pa+=0x1000){
        	if(dmmu_map_L2Pt_entry(table2_pa, i, page_pa + guest_pstart,  small_attrs))
    ...
    dmmu_create_L2Pt(table2_pa);

    dmmu_map_L1Pt_pt(guest_pstart, table2_pa, page_attrs);
    dmmu_map_L1Pt_pt(guest_vstart, table2_pa, page_attrs);
    ...
    for(i = 0; i < 256;i++, page_pa+=0x1000){
      	if(i >=1 && i <=7){
      		uint32_t ro_attrs = 0xE | (MMU_AP_USER_RO <<  MMU_L2_SMALL_AP_SHIFT);
       		dmmu_map_L2Pt_entry(table2_pa, i, page_pa, ro_attrs);
       	}
       	else
       		dmmu_map_L2Pt_entry(table2_pa, i, page_pa,  small_attrs);
    }
