...
//  linux_init();
    ...
    vm_0.config->firmware = get_guest(0);

    vm_1.config = &(minimal_config);
    vm_1.config->firmware = get_guest(1);

    printf("Guest1 config:\n");
    printf("vstart: %x, pstart %x, psize %x\n",
    		vm_1.config->firmware->vstart,
			vm_1.config->firmware->pstart,
			vm_1.config->firmware->psize);

    printf("Guest0 config:\n");
    printf("vstart: %x, pstart %x, psize %x\n",
    		vm_0.config->firmware->vstart,
			vm_0.config->firmware->pstart,
			vm_0.config->firmware->psize);
...
//    dump_mmu(guest_pt_va); // DEBUG
   ...
   // printf("SP: %x, PSR: %x\n", curr_vm->mode_states[HC_GM_TRUSTED].ctx.sp, curr_vm->mode_states[HC_GM_TRUSTED].ctx.psr);
#endif
