...
/* don't use this, this does not work yet!! */
char* fgets(char *buf, int len)
{
	...
	/* get len bytes or until newline */
	for (p = buf, len--; len > 0; len--) {
		c = read_getc();
		*p++ = c;
...
char * fgets_print(char *buf, int len)
{
	int c;
		char *p;
